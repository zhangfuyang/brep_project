...
class Solid3DModel_v2(nn.Module):
    def __init__(self, config):
        ...
        self.cond_solid_model = UNet3DModel(in_channels, out_channels, freq_shift,
            ...
            if cross_params['s2f_model'][i]:
                ...
                self.s2f_model.append(
                    CrossEncodeBlock3D(
                        ...
                        temb_channels=self.cond_solid_model.time_embed_dim,
                        num_layers=layers_per_block,
                        resnet_act_fn=act_fn,
                        num_groups=norm_num_groups,
                        eps=norm_eps,
    ...
    def forward(self, faces, solid, timestep):
        ...
        solid_t_emb = self.cond_solid_model.time_encode(timestep, solid.shape[0], solid.device)
        solid_latent = self.cond_solid_model.conv_in_model(solid) # bs, ch, n, n, n
        ...
        for encode_block_i in range(len(self.face_model.encode_blocks)):
            solid_latent, solid_res_samples = self.cond_solid_model.block_forward(
                self.cond_solid_model.encode_blocks[encode_block_i], solid_latent, solid_t_emb)
        ...
        for mid_block_i in range(len(self.face_model.mid_block)):
            solid_latent, _ = self.cond_solid_model.block_forward(
                self.cond_solid_model.mid_block[mid_block_i], solid_latent, solid_t_emb)
        ...
        for decode_block_i in range(len(self.face_model.decode_blocks)):
            solid_decode_block = self.cond_solid_model.decode_blocks[decode_block_i]
            ...
            solid_latent = self.cond_solid_model.block_forward(
...
class Solid3DModel(nn.Module):
    ...
    def forward(self, faces, solid, point, timestep):
        ...
        if hasattr(self.cond_solid_model, "conv_norm_out"):
            solid_latent = self.cond_solid_model.conv_out_model(solid_latent)
