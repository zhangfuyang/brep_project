...
class DiffusionExperiment(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        model_output_face = self.diffusion_model(
                            face_latent_noisy, solid_latent, t)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for i, t in enumerate(timesteps):
            ...
            model_output_face = self.diffusion_model(
                                z_faces_input, solid_latent, timestep)
            ...
            if batch_idx == 0: 
                for i in range(min(400, x.shape[0])):
                    if self.config['visual_type'] == 'vanilla':
                        ...
                        solid_voxel, face_voxel = self.latent_to_voxel(solid_latent[i][0], z_faces[i])
                        ...
                        self.save_result(solid_latent[i][0], z_faces[i], save_name)
