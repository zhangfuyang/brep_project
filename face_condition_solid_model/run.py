...
parser.add_argument('--config', type=str, default='face_condition_solid_model/configs/train.yaml')
...
if config['exp_params']['pretrained_model_path'] is not None:
    checkpoint = torch.load(config['exp_params']['pretrained_model_path'], map_location='cpu')
    state_dict = checkpoint['state_dict']
    new_state_dict = {}
    for k, v in state_dict.items():
        if k.startswith('diffusion_model.face_model.'):
            new_state_dict[k] = v
