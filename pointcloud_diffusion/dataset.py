...
class LatentDataset(torch.utils.data.Dataset):
    def __init__(self, data_config, split):
        ...
        if split == 'train':
            ...
            self.pointcloud_path = data_config['train_pointcloud_path']
            filter_path = data_config['train_filter_data_path']
        else:
            data_path = data_config['val_data_pkl_path']
            self.pointcloud_path = data_config['val_pointcloud_path']
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            pointcloud_path = os.path.join(self.pointcloud_path, 
                                           data_name, f'{solid_name}_surf_point.npy')
            pointcloud = np.load(pointcloud_path, allow_pickle=True).tolist()
            pointcloud = pointcloud['solid_surface_point']
        except:
...
if __name__ == "__main__":
    ...
    train_dataset = PartialPCLatentDataset(config['data_params'],'val')
    for _ in range(3):
