...
class DiffusionExperiment(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.config['generate_solid'] is False:
            ...
            noise_pred_face, _ = self.diffusion_model(noise_z, z[:,:1],t)
            sd_loss = torch.nn.functional.mse_loss(noise_pred_face, noise)
        else:
            # generate both faces and solid
            noise = torch.randn_like(z)
            noise_z = self.scheduler.add_noise(z, noise, t)
            noise_pred_face, noise_pred_solid = self.diffusion_model(noise_z[:,1:], noise_z[:,:1],t)
            noise_pred = torch.cat([noise_pred_solid, noise_pred_face], 1)
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.Adam(
            [
                {'params': self.diffusion_model.solid_unet.parameters(), 'lr': self.config['lr']},
                {'params': self.diffusion_model.face_unet.parameters(), 'lr': self.config['lr']},
...
class DiffusionExperiment_v2(pl.LightningModule):
    def __init__(self, config, diffusion_model):
        super(DiffusionExperiment_v2, self).__init__()
        self.config = config
        self.diffusion_model = diffusion_model
        self.scheduler = DDIMScheduler(
            num_train_timesteps=1000,
    ...
    @torch.no_grad()
    def preprocess(self, batch):
        sdf_voxel = batch['sdf_voxel'] # bs, 1, 1, N, N, N 
        face_voxel = batch['face_voxel'] # bs, M, 1, N, N, N 
