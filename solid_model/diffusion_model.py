...
class Solid3DModel_v2(nn.Module):
    ...
    def forward(self, solid, timestep):
        solid = solid[:,0]
        # solid: bs, ch, n, n, n
        # faces: bs, m, ch, n, n, n
        ...
        layer_idx = 0
        # down
        for encode_block_i in range(len(self.solid_model.encode_blocks)):
        ...
        # mid
        for mid_block_i in range(len(self.solid_model.mid_block)):
            solid_latent, _ = self.solid_model.block_forward(
                self.solid_model.mid_block[mid_block_i], solid_latent, solid_t_emb)
            layer_idx += 1
        
        # up
        for decode_block_i in range(len(self.solid_model.decode_blocks)):
