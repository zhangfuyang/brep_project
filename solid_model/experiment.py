...
class DiffusionExperiment(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        model_output_solid = self.diffusion_model(solid_latent_noisy, t)

        solid_target = solid_latent if self.config['prediction_type'] == 'sample' else noise_solid
        
        target = solid_target
        model_output = model_output_solid
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for i, t in enumerate(timesteps):
            ...
            timestep = torch.cat([t.unsqueeze(0)]*z_solid_input.shape[0], 0)
            model_output_solid = self.diffusion_model(z_solid_input, timestep)
