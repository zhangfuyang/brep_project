...
from experiment import SingleDiffusionExperiment
...
vqvae_model = load_model(VQVAE3D, 
                        config['exp_params']['vae_model']['config'], 
                        config['exp_params']['vae_model']['pretrained_path'],
                        exclude_prefix='vae_model.')
model = UNet3DModel(**config['model_params'])

config['exp_params']['phase'] = 'face' if \
                config['data_params']['data_key'] == 'face_bounded_distance_field' else 'solid'
experiment = SingleDiffusionExperiment(config['exp_params'], model, vqvae_model)
...
train_dataset = SingleDataset(config['data_params'], 'val' if config['data_params']['debug'] else 'train')
...
val_dataset = SingleDataset(config['data_params'], 'val')
