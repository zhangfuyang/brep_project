...
class SingleDiffusionExperiment(pl.LightningModule):
    def __init__(self, config, diffusion_model, vqvae_model):
        super(SingleDiffusionExperiment, self).__init__()
        self.config = config
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.trainer.is_global_zero:
            if batch_idx == 0: 
                for i in range(min(5, x.shape[0])):
                    voxel_gt = self.latent_to_voxel(x[i])
                    voxel_pred = self.latent_to_voxel(z[i])

                    save_name_prefix = os.path.join(self.logger.log_dir, 'images', 
                                         f'{self.global_step}_gt')
                    phase = 'sdf' if self.config['phase'] == 'solid' else 'face'
                    self.render_mesh(voxel_gt, save_name_prefix+f'_{i}.obj', phase=phase)

                    save_name_prefix = os.path.join(self.logger.log_dir, 'images', 
                                             f'{self.global_step}_gen')
                    self.render_mesh(voxel_pred, save_name_prefix+f'_{i}.obj', phase=phase)


    def latent_to_voxel(self, latent):
        latent = latent[None] # 1, C, N, N, N
        latent = latent * self.std + self.mean

        with torch.no_grad():
            voxel = self.vqvae_model.quantize_decode(latent) # 1, 1, N, N, N
